...
export interface GameTab {
  ...
  /** Saved account this tab signs in with, if any. */
  accountId?: string;
  /** Something happened here while the tab was in the background. */
  hasActivity?: boolean;
  /** Bumped to force a fresh mount of the game frame. */
  reloadNonce?: number;
  /**
   * A character is signed in right now. Deliberately not persisted:
   * characterName survives a restart, so counting it would report accounts as
   * connected before the game has even loaded.
   */
  isSignedIn?: boolean;
  isActive: boolean;
  isLoading: boolean;
...
interface GameTabState {
  ...
  setTabAccount: (id: string, accountId: string | undefined) => void;
  setTabSignedIn: (id: string, signedIn: boolean) => void;
  reloadTab: (id: string) => void;
  markActivity: (id: string) => void;
  connectedCount: () => number;
  clearAccountFromTabs: (accountId: string) => void;
...
export const useGameTabStore = create<GameTabState>()(
  persist(
    (set, get) => {
      ...
      setActiveTab: (id) => {
        set((state) => ({
          tabs: state.tabs.map((t) => ({
            ...t,
            isActive: t.id === id,
            // Looking at a tab is what acknowledges its activity.
            hasActivity: t.id === id ? false : t.hasActivity,
          })),
          activeTabId: id,
        }));
      },
      ...
      /**
       * Remounts a tab's game frame. A frozen game used to mean quitting the
       * whole app, which disconnects every other account too.
       */
      reloadTab: (id) => {
        set((state) => ({
          tabs: state.tabs.map((t) =>
      ...
      markActivity: (id) => {
        set((state) => ({
          tabs: state.tabs.map((t) =>
            t.id === id && state.activeTabId !== id ? { ...t, hasActivity: true } : t,
          ),
        }));
      },

      connectedCount: () => get().tabs.filter((t) => t.isSignedIn).length,
      ...
      setTabSignedIn: (id, signedIn) => {
        set((state) => ({
          tabs: state.tabs.map((t) => (t.id === id ? { ...t, isSignedIn: signedIn } : t)),
        }));
      },

      setTabAccount: (id, accountId) => {
        set((state) => ({
          tabs: state.tabs.map((t) => {
            if (t.id === id) return { ...t, accountId };
            // An account drives at most one tab, otherwise both tabs would race
            // to sign in with the same credentials.
            if (accountId && t.accountId === accountId) {
              const { accountId: _dropped, ...rest } = t;
              return rest;
            }
            return t;
          }),
      ...
      clearAccountFromTabs: (accountId) => {
        set((state) => ({
          tabs: state.tabs.map((t) => {
