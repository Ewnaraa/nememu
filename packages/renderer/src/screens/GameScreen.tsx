import { useEffect, useRef, useState, useCallback, type CSSProperties } from 'react'
import { useSettings } from '@/App'
import { Plus, X, Settings, Minus, Square, Copy, RotateCw, Keyboard } from 'lucide-react'
import { WindowButton } from '@/components/WindowButton'
import { ShortcutsOverlay } from '@/components/ShortcutsOverlay'
import { PortWarning } from '@/components/PortWarning'
import { useGameTabStore, GameTab } from '@/stores/gameTabStore'
import { useSettingsStore } from '@/stores/settingsStore'
import { useTeamStore } from '@/stores/teamStore'
import { useHotkeys, attachHotkeys } from '@/hooks/use-hotkeys'
import { initAutoGroup, broadcastLeaderPosition, destroyAutoGroup, sendPartyInvite, autoAcceptPartyInvite } from '@/mods/auto-group'
import { initNotificationFocus } from '@/mods/notification-focus'
import { restoreSession } from '@/mods/account-session'
import { endTurn, selectSpellSlot, toggleReady } from '@/mods/combat'
import { closeOpenWindows, switchGameWindow } from '@/mods/game-windows'
import { changeMapDirection } from '@/mods/map-travel'
import { focusChat } from '@/mods/chat'
import { startFpsMeter } from '@/mods/fps-meter'
import { setDisplayOption, toggleDisplayOption, type DisplayToggle } from '@/mods/display-toggles'
import { toggleGameOption, toggleGrid } from '@/mods/game-options'
import { useAccountStore } from '@/stores/accountStore'
import { colors } from '@/theme'
import { DofusWindow, HTMLIFrameElementWithDofus } from '@/types/dofus-window'
import { captureCharacterIcon } from '@/utils/capture-icon'
import type { HotkeyAction } from '@nememu/shared'
import type { MapDirection } from '@/types/dofus-window'
import logoImg from '@/assets/logo.png'
import loadingBgImg from '@/assets/game-loading-bg.jpg'
import { useT } from '@/i18n'

const TITLEBAR_HEIGHT = 32
const MAX_POLL_ATTEMPTS = 50
const POLL_INTERVAL = 200
const RESIZE_DELAYS = [100, 250, 500, 1000, 2000]
const PARTY_INVITE_DELAY = 3000

/** Display toggles driven by holding a key: pressed shows, released hides. */
const HELD_DISPLAY_TOGGLES: Record<string, DisplayToggle> = {
  'toggle-monster-info': 'monster-info'
}

/**
 * Hotkey action to the game window it opens, read from its addWindow calls.
 * Several windows are tabbed containers — the grimoire holds spells, quests and
 * jobs, the social window holds friends and guild — so the tab is given too,
 * otherwise the shortcut lands on a menu instead of the thing asked for.
 */
/** Arrow keys, mapped onto the game's own map edges. */
const MAP_DIRECTIONS: Record<string, MapDirection> = {
  'map-left': 'left',
  'map-right': 'right',
  'map-up': 'top',
  'map-down': 'bottom'
}

const GAME_WINDOWS: Record<string, { id: string; params?: Record<string, unknown> }> = {
  'window-inventory': { id: 'equipment' },
  'window-characteristics': { id: 'characteristics' },
  'window-map': { id: 'worldMap' },
  'window-options': { id: 'options' },
  'window-arena': { id: 'arena' },
  'window-mount': { id: 'mount' },
  'window-grimoire': { id: 'grimoire', params: { tabId: 'spells' } },
  'window-quests': { id: 'grimoire', params: { tabId: 'quests' } },
  'window-jobs': { id: 'grimoire', params: { tabId: 'jobs' } },
  'window-social': { id: 'social', params: { tabId: 'friends' } },
  'window-guild': { id: 'social', params: { tabId: 'guild' } }
}


declare global {
  interface Window {
    $gameWindows: DofusWindow[]
    $game_id: string
    $current_id: string
    $appSchemeLinkCalled: (payload: string) => void
  }
}

const loadingBackdropStyle: CSSProperties = {
  position: 'absolute',
  inset: 0,
  overflow: 'hidden',
  background: '#07080c'
}

function GameLoadingBackdrop({ title, subtitle }: { title: string; subtitle: string }) {
  const t = useT()

  return (
    <div style={loadingBackdropStyle}>
      <div
        style={{
          position: 'absolute',
          inset: 0,
          backgroundImage: `url(${loadingBgImg})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center center',
          transform: 'scale(1.05)',
          filter: 'saturate(1.02)'
        }}
      />
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(180deg, rgba(7,8,12,0.18) 0%, rgba(7,8,12,0.6) 42%, rgba(7,8,12,0.92) 100%)'
        }}
      />
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(90deg, rgba(7,8,12,0.88) 0%, rgba(7,8,12,0.36) 44%, rgba(7,8,12,0.82) 100%)'
        }}
      />
      <div
        style={{
          position: 'absolute',
          left: '8%',
          top: '50%',
          transform: 'translateY(-50%)',
          width: 'min(420px, 78vw)',
          padding: '24px 24px 22px',
          borderRadius: 22,
          border: '1px solid rgba(255,255,255,0.08)',
          background: 'linear-gradient(180deg, rgba(8,10,16,0.74) 0%, rgba(8,10,16,0.86) 100%)',
          boxShadow: '0 24px 70px rgba(0,0,0,0.45)',
          backdropFilter: 'blur(8px)'
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          <img src={logoImg} alt="" style={{ width: 44, height: 44, filter: 'drop-shadow(0 0 18px rgba(201,162,77,0.4))' }} />
          <div>
            <div style={{ fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(201,162,77,0.92)', fontWeight: 700 }}>
              Nememu
            </div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)' }}>{t('Preparing the client')}</div>
          </div>
        </div>

        <div style={{ fontSize: 34, lineHeight: 1.05, fontWeight: 800, color: '#fff', marginBottom: 10 }}>
          {title}
        </div>
        <div style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(255,255,255,0.64)', marginBottom: 18 }}>
          {subtitle}
        </div>

        <div
          style={{
            height: 10,
            borderRadius: 999,
            background: 'rgba(255,255,255,0.1)',
            overflow: 'hidden'
          }}
        >
          <div
            style={{
              width: '42%',
              height: '100%',
              borderRadius: 999,
              background: 'linear-gradient(90deg, rgba(201,162,77,0.62) 0%, rgba(232,199,106,0.98) 100%)',
              boxShadow: '0 0 24px rgba(201,162,77,0.28)',
              animation: 'nememu-pulse 2s ease-in-out infinite',
              transformOrigin: 'left center'
            }}
          />
        </div>
      </div>
    </div>
  )
}


/**
 * Signs a tab in with its saved account, when it has one. Every failure path is
 * silent by design: the game simply shows its normal login screen, which is a
 * far better outcome than a stuck tab.
 */
async function signInWithSavedAccount(gameWindow: DofusWindow, tabId: string) {
  const linkedAccountId = useGameTabStore.getState().tabs.find((t) => t.id === tabId)?.accountId
  if (!linkedAccountId) return

  try {
    // The tab, not the account: the main process resolves the link itself and
    // will only ever answer with the credentials of this tab's own account.
    const secrets = await window.nememu.getSecretsForTab(tabId)
    if (!secrets) {
      window.nememu.logger.warn(`No stored credentials for the account linked to tab ${tabId}.`)
      return
    }

    const signedIn = await restoreSession(gameWindow, secrets)
    window.nememu.logger.info(
      signedIn
        ? `Auto-login sent for tab ${tabId}.`
        : `Auto-login unavailable for tab ${tabId}, showing the login screen.`
    )
  } catch (err) {
    window.nememu.logger.warn('Auto-login failed', err)
  }
}

function GameIframe({
  tab,
  gameSrc,
  isVisible,
  onHotkeyAction,
  onHotkeyRelease
}: {
  tab: GameTab
  gameSrc: string
  isVisible: boolean
  onHotkeyAction: (action: HotkeyAction, source: DofusWindow) => void
  onHotkeyRelease: (action: HotkeyAction, source: DofusWindow) => void
}) {
  const t = useT()
  const [fps, setFps] = useState<number | null>(null)
  const showFps = useSettingsStore((s) => s.game.showFpsCounter)
  const iframeRef = useRef<HTMLIFrameElementWithDofus>(null)
  const cleanupRef = useRef<Array<() => void>>([])
  const { setTabReady, setTabLoading, setTabCharacter, setTabSignedIn } = useGameTabStore()
  const onHotkeyActionRef = useRef(onHotkeyAction)
  onHotkeyActionRef.current = onHotkeyAction
  const onHotkeyReleaseRef = useRef(onHotkeyRelease)
  onHotkeyReleaseRef.current = onHotkeyRelease

  const cleanupGameListeners = () => {
    for (const cleanup of cleanupRef.current) cleanup()
    cleanupRef.current = []

    // The frame registers itself in $gameWindows; without this the array grows
    // stale entries on every reload, and lookups by tab id find dead windows.
    const registry = window.parent.$gameWindows
    if (registry) {
      window.parent.$gameWindows = registry.filter((gw) => gw.$game_id !== tab.id)
    }
  }

  useEffect(() => cleanupGameListeners, [])

  const handleLoad = () => {
    if (!iframeRef.current) return
    cleanupGameListeners()

    const gameWindow = iframeRef.current.contentWindow
    setTabReady(tab.id, false)
    setTabLoading(tab.id, true)

    gameWindow.openDatabase = undefined
    gameWindow.initDofus(() => {
      window.nememu.logger.info('initDofus done for tab', tab.id)

      if (!window.parent.$gameWindows) {
        window.parent.$gameWindows = []
      }
      gameWindow.$game_id = tab.id
      window.parent.$current_id = tab.id
      window.parent.$gameWindows.push(gameWindow)

      setTabReady(tab.id, true)
      setTabLoading(tab.id, false)

      void signInWithSavedAccount(gameWindow, tab.id)

      // The game owns its own document, so shell-level key listeners never see
      // these presses. Bind the same hotkeys inside the game window too.
      cleanupRef.current.push(
        attachHotkeys(gameWindow as unknown as Window, () => ({
          hotkeys: useSettingsStore.getState().hotkeys,
          onAction: (action) => onHotkeyActionRef.current(action, gameWindow),
          onRelease: (action) => onHotkeyReleaseRef.current(action, gameWindow)
        }))
      )

      const kickResize = () => {
        try {
          gameWindow.dispatchEvent(new Event('resize'))
          gameWindow.gui?._resizeUi?.()
        } catch {}
      }

      kickResize()
      RESIZE_DELAYS.forEach((ms) => setTimeout(kickResize, ms))

      const observer = new ResizeObserver(kickResize)
      if (iframeRef.current) observer.observe(iframeRef.current)
      cleanupRef.current.push(() => observer.disconnect())

      const gw = gameWindow as any

      const attachGameListeners = () => {
        if (!gw.gui?.playerData || !gw.dofus?.connectionManager) return false

        gw.gui.playerData.on('characterSelectedSuccess', () => {
          const name = gw.gui.playerData.characterBaseInformations?.name
          if (name) {
            setTabCharacter(tab.id, name)
            setTabSignedIn(tab.id, true)

            const teamState = useTeamStore.getState()
            const matchedChar = teamState.getCharacterByName(name)
            if (matchedChar) {
              teamState.linkCharacterToTab(matchedChar.id, tab.id)


              const settings = useSettingsStore.getState()
              if (settings.game.autoGroupEnabled && settings.game.autoInviteEnabled && teamState.activeTeamId) {
                const team = teamState.getTeam(teamState.activeTeamId)
                if (team && team.memberIds.includes(matchedChar.id)) {
                  const leader = teamState.getCharacter(team.leaderId)

                  if (matchedChar.id !== team.leaderId && leader) {

                    autoAcceptPartyInvite(gw, leader.name)


                    const leaderTabId = teamState.getTabForCharacter(team.leaderId)
                    if (leaderTabId) {
                      const leaderGw = window.$gameWindows?.find((gw) => gw.$game_id === leaderTabId)
                      if (leaderGw) {
                        setTimeout(() => sendPartyInvite(leaderGw, name), PARTY_INVITE_DELAY)
                      }
                    }
                  }
                }
              }
            }
          }

          const look = gw.gui.playerData.characterBaseInformations?.entityLook
          if (!gw.CharacterDisplay || !look) return

          const charDisplay = new gw.CharacterDisplay({ scale: 'fitin' })
          charDisplay.setLook(look, {
            riderOnly: true, direction: 4, animation: 'AnimArtwork',
            boneType: 'timeline/', skinType: 'timeline/'
          })
          charDisplay.rootElement.style.cssText = 'position:absolute;left:-9999px;width:128px;height:128px;'
          gw.document.body.appendChild(charDisplay.rootElement)

          captureCharacterIcon(charDisplay, gw.document, (dataUrl) => {
            const charName = gw.gui.playerData.characterBaseInformations?.name
            if (charName) window.nememu.saveCharacterImage(charName, dataUrl)
            window.top?.postMessage({ type: 'nememu:char-icon', tabId: tab.id, dataUrl }, '*')
          })
        })

        cleanupRef.current.push(
          startFpsMeter(gameWindow, (value) => {
            // Only pay for a re-render when the number actually moves.
            setFps((previous) => (previous === value ? previous : value))
          })
        )

        cleanupRef.current.push(initNotificationFocus(gameWindow, tab.id, {
          shouldNotify: () => useSettingsStore.getState().game.notificationsEnabled,
          isActiveTab: (tabId) => useGameTabStore.getState().activeTabId === tabId,
          focusTab: (tabId) => useGameTabStore.getState().setActiveTab(tabId),
          markActivity: (tabId) => useGameTabStore.getState().markActivity(tabId),
          markDisconnected: (tabId) => useGameTabStore.getState().setTabSignedIn(tabId, false)
        }))

        return true
      }

      if (!attachGameListeners()) {
        let attempts = 0
        const poll = setInterval(() => {
          if (attachGameListeners() || ++attempts > MAX_POLL_ATTEMPTS) clearInterval(poll)
        }, POLL_INTERVAL)
      }
    })
  }

  return (
    <div
      style={{
        position: 'absolute',
        top: 0,
        left: 0,
        right: 0,
        bottom: 0,
        display: isVisible ? 'block' : 'none'
      }}
    >
      <iframe
        ref={iframeRef}
        onLoad={handleLoad}
        src={gameSrc + '?id=' + tab.id}
        style={{
          border: 'none',
          width: '100%',
          height: '100%',
          position: 'absolute',
          top: 0,
          left: 0,
          display: 'block'
        }}
      />
      {showFps && fps !== null && tab.isReady && (
        <div
          style={{
            position: 'absolute',
            top: 6,
            right: 8,
            padding: '2px 7px',
            borderRadius: 4,
            background: 'rgba(0,0,0,0.55)',
            color: fps >= 50 ? '#7ddc7d' : fps >= 30 ? colors.accentText : colors.danger,
            fontFamily: 'var(--font-mono)',
            fontSize: 11,
            lineHeight: 1.6,
            pointerEvents: 'none',
            userSelect: 'none'
          }}
        >
          {fps} FPS
        </div>
      )}
      {!tab.isReady && (
        <GameLoadingBackdrop
          title={t('Loading game assets')}
          subtitle={t('Opening {name}. The game screen will appear as soon as the client finishes booting.', { name: tab.characterName || tab.name })}
        />
      )}
    </div>
  )
}

export function GameScreen() {
  const t = useT()
  const { tabs, activeTabId, addTab, removeTab, setActiveTab, canAddTab, reorderTabs, reloadTab } = useGameTabStore()
  const { hotkeys, game, loadSettings, isHydrated, shortcutsSeen, markShortcutsSeen } = useSettingsStore()
  const { activeTeamId, teams, characterTabMap } = useTeamStore()
  const { setSettingsOpen } = useSettings()
  const [gameSrc, setGameSrc] = useState('')
  const [isMaximized, setIsMaximized] = useState(false)
  const [dragTabId, setDragTabId] = useState<string | null>(null)
  const [dragOverTabId, setDragOverTabId] = useState<string | null>(null)
  const [shortcutsOpen, setShortcutsOpen] = useState(false)

  useEffect(() => {
    if (!isHydrated) loadSettings()
  }, [isHydrated, loadSettings])

  // Personne ne devine des raccourcis dans un jeu qui n'en a pas : la feuille
  // s'ouvre d'elle-meme au tout premier lancement, puis plus jamais.
  useEffect(() => {
    if (!isHydrated || shortcutsSeen) return
    setShortcutsOpen(true)
    markShortcutsSeen()
  }, [isHydrated, shortcutsSeen, markShortcutsSeen])

  useEffect(() => {
    window.nememu.setConnectedTabs(tabs.filter((t) => t.isSignedIn).length)
  }, [tabs])

  useEffect(() => {
    const active = tabs.find((t) => t.id === activeTabId)
    window.nememu.setWindowTitle(active?.characterName ?? '')
  }, [tabs, activeTabId])

  useEffect(() => {
    void useAccountStore.getState().load()
    return window.nememu.onAccountsChanged((accounts) => {
      useAccountStore.getState().setAccounts(accounts)
    })
  }, [])

  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'nememu:char-icon') {
        useGameTabStore.getState().setTabIcon(e.data.tabId, e.data.dataUrl)
      }
    }
    window.addEventListener('message', handler)
    return () => window.removeEventListener('message', handler)
  }, [])


  useEffect(() => {
    const teamState = useTeamStore.getState()
    const tabIds = new Set(tabs.map((t) => t.id))
    for (const [, tid] of Object.entries(teamState.characterTabMap)) {
      if (!tabIds.has(tid)) teamState.unlinkTab(tid)
    }
  }, [tabs])

  useEffect(() => {
    const unsub = window.nememu.onAuthCallback((url) => {
      const iframes = document.querySelectorAll('iframe')
      for (const iframe of iframes) {
        try {
          const win = (iframe as HTMLIFrameElement).contentWindow as any
          if (win?.$appSchemeLinkCalled) {
            win.$appSchemeLinkCalled(url)
            return
          }
        } catch {}
      }
    })
    return unsub
  }, [])

  useEffect(() => {
    return window.nememu.onNativeNotificationClick((tabId) => {
      if (tabId) useGameTabStore.getState().setActiveTab(tabId)
    })
  }, [])

  useEffect(() => {
    window.nememu.fetchGameContext().then((ctx) => {
      window.buildVersion = ctx.buildVersion
      window.appVersion = ctx.appVersion
      window.appInfo = { version: ctx.appVersion }
      ;(window as typeof window & { platform?: string }).platform = ctx.platform
      setGameSrc(ctx.gameSrc)
    })
  }, [])

  const handleHotkeyRelease = useCallback((action: HotkeyAction, source?: DofusWindow) => {
    const heldToggle = HELD_DISPLAY_TOGGLES[action]
    if (!heldToggle) return

    const gameWindow =
      source ?? window.$gameWindows?.find((gw) => gw.$game_id === useGameTabStore.getState().activeTabId)

    if (gameWindow) setDisplayOption(gameWindow, heldToggle, false)
  }, [])

  const handleHotkeyAction = useCallback(
    (action: HotkeyAction, source?: DofusWindow) => {
      const tabStore = useGameTabStore.getState()
      const currentTabs = tabStore.tabs
      const currentActiveId = tabStore.activeTabId

      // Combat shortcuts act on the window the key came from, falling back to
      // the visible tab when they were pressed on the shell itself.
      const gameWindow =
        source ?? window.$gameWindows?.find((gw) => gw.$game_id === currentActiveId)

      const target = GAME_WINDOWS[action]
      if (target) {
        if (gameWindow) switchGameWindow(gameWindow, target.id, target.params)
        return
      }

      if (action === 'focus-chat') {
        if (gameWindow) focusChat(gameWindow)
        return
      }

      const direction = MAP_DIRECTIONS[action]
      if (direction) {
        if (gameWindow) changeMapDirection(gameWindow, direction)
        return
      }

      if (action === 'toggle-fullscreen') {
        window.nememu.toggleFullScreen()
        return
      }

      if (action === 'toggle-spell-effects') {
        if (gameWindow) toggleGameOption(gameWindow, 'allowSpellEffects')
        return
      }

      if (action === 'toggle-grid') {
        if (gameWindow) toggleGrid(gameWindow)
        return
      }

      if (action === 'reload-tab') {
        if (currentActiveId) tabStore.reloadTab(currentActiveId)
        return
      }

      const displayToggles: Record<string, DisplayToggle> = {
        'toggle-tactical-mode': 'tactical-mode',
        'toggle-creature-mode': 'creature-mode',
        'toggle-transparent-mode': 'transparent-mode',
        'toggle-interactives': 'interactives',
        'toggle-map-info': 'map-info',
        'toggle-nicknames': 'nicknames'
      }

      const displayToggle = displayToggles[action]
      if (displayToggle) {
        if (gameWindow) toggleDisplayOption(gameWindow, displayToggle)
        return
      }

      const heldToggle = HELD_DISPLAY_TOGGLES[action]
      if (heldToggle) {
        if (gameWindow) setDisplayOption(gameWindow, heldToggle, true)
        return
      }

      if (action === 'close-windows') {
        if (gameWindow) closeOpenWindows(gameWindow)
        return
      }

      if (action === 'fight-ready') {
        if (gameWindow) toggleReady(gameWindow)
        return
      }

      if (action === 'fight-end-turn') {
        if (gameWindow) endTurn(gameWindow)
        return
      }

      if (action.startsWith('spell-')) {
        const slot = parseInt(action.slice('spell-'.length), 10)
        if (gameWindow && Number.isFinite(slot)) selectSpellSlot(gameWindow, slot - 1)
        return
      }

      switch (action) {
        case 'switch-tab-1':
        case 'switch-tab-2':
        case 'switch-tab-3':
        case 'switch-tab-4':
        case 'switch-tab-5': {
          const index = parseInt(action.replace('switch-tab-', ''), 10) - 1
          if (currentTabs[index]) tabStore.setActiveTab(currentTabs[index].id)
          break
        }
        case 'new-tab':
          if (tabStore.canAddTab()) tabStore.addTab()
          break
        case 'close-tab':
          if (currentActiveId) tabStore.removeTab(currentActiveId)
          break
        case 'toggle-mute':
          useSettingsStore.getState().toggleAudioMute()
          break
        case 'toggle-notifications':
          useSettingsStore.getState().toggleNotifications()
          break
        case 'next-tab': {
          const currentIdx = currentTabs.findIndex((t) => t.id === currentActiveId)
          const nextIdx = (currentIdx + 1) % currentTabs.length
          tabStore.setActiveTab(currentTabs[nextIdx].id)
          break
        }
        case 'prev-tab': {
          const currentIdx = currentTabs.findIndex((t) => t.id === currentActiveId)
          const prevIdx = (currentIdx - 1 + currentTabs.length) % currentTabs.length
          tabStore.setActiveTab(currentTabs[prevIdx].id)
          break
        }
        case 'zoom-in':
          window.nememu.stepZoom(1)
          break
        case 'zoom-out':
          window.nememu.stepZoom(-1)
          break
        case 'zoom-reset':
          window.nememu.resetZoom()
          break
      }
    },
    []
  )

  useHotkeys({
    hotkeys,
    onAction: handleHotkeyAction,
    onRelease: handleHotkeyRelease,
    enabled: true
  })


  useEffect(() => {
    if (!game.autoGroupEnabled || !activeTeamId) {
      destroyAutoGroup()
      return
    }

    const team = teams.find((t) => t.id === activeTeamId)
    if (!team || !team.leaderId) return

    const leaderTabId = characterTabMap[team.leaderId] ?? null
    if (!leaderTabId) return

    const followerTabIds = team.memberIds
      .filter((id) => id !== team.leaderId)
      .map((id) => characterTabMap[id])
      .filter((tabId): tabId is string => !!tabId)

    const gameWindows = window.$gameWindows
    if (!gameWindows || gameWindows.length === 0) return

    const cleanups: Array<() => void> = []
    for (const gw of gameWindows) {
      const cleanup = initAutoGroup(gw, gw.$game_id, {
        enabled: true,
        leaderTabId,
        leaderMapId: null,
        leaderPosition: null,
        followerTabIds
      }, {
        onLeaderMapChange: (mapId, position) => {
          broadcastLeaderPosition(mapId, position)
        },
        onFollowerMoved: () => {}
      })
      cleanups.push(cleanup)
    }

    return () => {
      for (const fn of cleanups) fn()
    }
  }, [game.autoGroupEnabled, activeTeamId, teams, tabs, characterTabMap])

  if (!gameSrc) {
    return (
      <div style={{ position: 'relative', flex: 1 }}>
        <GameLoadingBackdrop
          title={t('Starting Nememu')}
          subtitle={t('Loading the local game context and preparing the client shell.')}
        />
      </div>
    )
  }

  const handleDrop = (targetTabId: string) => {
    if (dragTabId && dragTabId !== targetTabId) {
      const oldIndex = tabs.findIndex((t) => t.id === dragTabId)
      const newIndex = tabs.findIndex((t) => t.id === targetTabId)
      const newOrder = tabs.map((t) => t.id)
      newOrder.splice(oldIndex, 1)
      newOrder.splice(newIndex, 0, dragTabId)
      reorderTabs(newOrder)
    }
    setDragTabId(null)
    setDragOverTabId(null)
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
          height: TITLEBAR_HEIGHT,
          background: colors.titlebar,
          borderBottom: `1px solid ${colors.brandBorder}`,
          flexShrink: 0,
          overflow: 'hidden',
          WebkitAppRegion: 'drag',
        } as React.CSSProperties}
      >
        {/* The mark is the way back to the launcher.
            It used to be decoration — the one part of the bar that was never a
            control. But the launcher now stays open behind the game and can be
            closed like any window, so something in the game has to be able to
            call it back, and the logo is where a player already looks for
            "home". It steps out of the drag region for that. */}
        <button
          onClick={() => window.nememu.showLauncher()}
          title={t('Open the launcher')}
          style={{
            display: 'flex', alignItems: 'center', padding: '0 9px 0 10px', flexShrink: 0,
            height: '100%', background: 'none', border: 'none', cursor: 'pointer',
            WebkitAppRegion: 'no-drag'
          } as React.CSSProperties}
          onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)' }}
          onMouseLeave={(e) => { e.currentTarget.style.background = 'none' }}
        >
          <img src={logoImg} alt="" style={{ width: 15, height: 15, opacity: 0.9 }} />
        </button>
        <div style={{ display: 'flex', alignItems: 'center', height: '100%', overflow: 'auto', WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          {tabs.map((tab) => (
            <button
              key={tab.id}
              draggable
              onClick={() => setActiveTab(tab.id)}
              onDragStart={(e) => {
                setDragTabId(tab.id)
                e.dataTransfer.effectAllowed = 'move'
              }}
              onDragOver={(e) => {
                e.preventDefault()
                e.dataTransfer.dropEffect = 'move'
                setDragOverTabId(tab.id)
              }}
              onDragEnter={(e) => { e.preventDefault(); setDragOverTabId(tab.id) }}
              onDragLeave={() => { if (dragOverTabId === tab.id) setDragOverTabId(null) }}
              onDrop={(e) => { e.preventDefault(); handleDrop(tab.id) }}
              onDragEnd={() => { setDragTabId(null); setDragOverTabId(null) }}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 6,
                padding: '0 12px',
                height: '100%',
                fontSize: 11.5,
                // A character name is a name, not a column of data. Monospace
                // here bought no alignment — the tabs are sized to content —
                // and read like a terminal, which is not what a tab strip full
                // of Ilymira and Aratee should look like.
                letterSpacing: '0.01em',
                border: 'none',
                borderRight: `1px solid ${colors.borderSubtle}`,
                cursor: 'pointer',
                whiteSpace: 'nowrap',
                background: tab.id === activeTabId ? colors.surfaceActive : 'transparent',
                color: tab.id === activeTabId ? colors.text : colors.textDim,
                opacity: dragTabId === tab.id ? 0.5 : 1,
                borderLeft: dragOverTabId === tab.id && dragTabId !== tab.id ? `2px solid ${colors.accent}` : undefined,
                transition: 'opacity 0.15s',
              }}
            >
              {tab.characterIcon && (
                <img src={tab.characterIcon} alt="" style={{ width: 20, height: 20, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
              )}
              <span
                style={{
                  maxWidth: 100,
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  // A tab whose session dropped is dimmed: the name is still
                  // there, but it no longer stands for a character in game.
                  opacity: tab.characterName && !tab.isSignedIn ? 0.45 : 1
                }}
              >
                {tab.characterName || tab.name}
              </span>
              {tab.characterName && !tab.isSignedIn && (
                <span
                  title={t('Disconnected — reload this tab to sign in again')}
                  style={{
                    width: 6,
                    height: 6,
                    borderRadius: '50%',
                    background: colors.danger,
                    flexShrink: 0
                  }}
                />
              )}
              {tab.hasActivity && tab.id !== activeTabId && (
                <span
                  title={t('Activity in this tab')}
                  style={{
                    width: 6,
                    height: 6,
                    borderRadius: '50%',
                    background: colors.accent,
                    boxShadow: `0 0 6px ${colors.accent}`,
                    flexShrink: 0
                  }}
                />
              )}
              {tab.id === activeTabId && (
                <span
                  title={t('Reload this tab')}
                  onClick={(e) => { e.stopPropagation(); reloadTab(tab.id) }}
                  style={{ opacity: 0.4, cursor: 'pointer', lineHeight: 1, display: 'flex' }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.opacity = '1' }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.opacity = '0.4' }}
                >
                  <RotateCw size={10} />
                </span>
              )}
              {tabs.length > 1 && (
                <span
                  title={t('Close this tab')}
                  onClick={(e) => { e.stopPropagation(); removeTab(tab.id) }}
                  style={{ opacity: 0.4, cursor: 'pointer', lineHeight: 1, display: 'flex' }}
                  // currentTarget, not target: the pointer usually lands on the
                  // icon's <path>, and styling that leaves the span dim.
                  onMouseEnter={(e) => { e.currentTarget.style.opacity = '1' }}
                  onMouseLeave={(e) => { e.currentTarget.style.opacity = '0.4' }}
                >
                  <X size={10} />
                </span>
              )}
            </button>
          ))}
          {canAddTab() && (
            <button
              onClick={() => addTab()}
              title={t('New tab')}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: '100%', background: 'none', border: 'none', color: colors.textDim, cursor: 'pointer' }}
            >
              <Plus size={13} />
            </button>
          )}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', height: '100%', WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          <button
            onClick={() => setShortcutsOpen(true)}
            title={t('Keyboard shortcuts')}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: '100%', background: 'none', border: 'none', color: colors.textMuted, cursor: 'pointer' }}
          >
            <Keyboard size={13} />
          </button>
          <button
            onClick={() => setSettingsOpen(true)}
            title={t('Settings')}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 28, height: '100%', background: 'none', border: 'none', color: colors.textMuted, cursor: 'pointer' }}
          >
            <Settings size={12} />
          </button>
          <WindowButton onClick={() => window.nememu.minimize()} title={t('Minimize')}>
            <Minus size={12} />
          </WindowButton>
          <WindowButton
            onClick={() => { window.nememu.maximize(); setIsMaximized(!isMaximized) }}
            title={isMaximized ? t('Restore') : t('Maximize')}
          >
            {isMaximized ? <Copy size={10} /> : <Square size={10} />}
          </WindowButton>
          <WindowButton onClick={() => window.nememu.close()} title={t('Close')} hoverBg={colors.dangerClose}>
            <X size={14} />
          </WindowButton>
        </div>
      </div>
      <PortWarning />
      <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
        {tabs.map((tab) => (
          <GameIframe
            key={`${tab.id}:${tab.reloadNonce ?? 0}`}
            tab={tab}
            gameSrc={gameSrc}
            isVisible={tab.id === activeTabId}
            onHotkeyAction={handleHotkeyAction}
            onHotkeyRelease={handleHotkeyRelease}
          />
        ))}
      </div>
      {shortcutsOpen && (
        <ShortcutsOverlay
          onClose={() => setShortcutsOpen(false)}
          onOpenSettings={() => setSettingsOpen(true)}
        />
      )}
    </div>
  )
}
